...
export function LatinSection({
    ...
    const checkIntroStatus = async () => {
      try {
        const data = await fetchApi<{ watched: boolean }>(
          '/api/basics/intro-video/complete?language=latin'
        )
        setHasWatchedIntro(data.watched)
      } catch (err) {
        // If check fails, assume not watched to be safe
        logger.error('LatinSection', 'Failed to check intro video status', {
          error: err,
        })
  ...
  const loadExercise = useCallback(async () => {
    ...
    try {
      ...
      }>('/api/basics/translation/exercise?language=latin')
      setExercise(data.exercise)
      setProgress(data.progress)
    } catch (err) {
      setError('Failed to load Latin exercise')
      logger.error('LatinSection', 'Failed to load Latin exercise', {
        error: err,
      })
    } finally {
      setIsLoading(false)
  ...
  const handleSkip = async () => {
    ...
    try {
      ...
      logger.error('LatinSection', 'Failed to skip', { error: err })
    }
  }
    ...
    return (
      <div className="animate-pulse space-y-4">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 b-bg-writing-light rounded" />
          <div className="h-6 bg-b-border rounded w-20" />
        </div>
        <div className="h-64 b-bg-writing-light rounded-lg" />
      </div>
    )
  }
  ...
  if (hasWatchedIntro === false) {
    return <IntroVideoGate language="latin" onComplete={handleIntroComplete} />
  }
